...
def get_adjacency_matrix(distance_df, sensor_ids, normalized_k=0.1):
    ...
    for row in distance_df.values:
        ...
        if str(int(row[0])) not in sensor_id_to_ind or str(int(row[1])) not in sensor_id_to_ind:
            continue
        # print(sensor_id_to_ind[row[0]], sensor_id_to_ind[row[1]], row[2])
        dist_mx[sensor_id_to_ind[str(int(row[0]))], sensor_id_to_ind[str(int(row[1]))]] = row[2]
